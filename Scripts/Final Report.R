...
Rows with missing or suppressed prevalence values were removed. "Measured Visual Acuity" was excluded as it is not a prevalence measure. Analysis was restricted to adults aged 40 and older (age groups with sufficient data) and to the three main racial/ethnic groups. Estimates reported for "Both sexes" were used.
...
ggplot(df, aes(x = Data_Value)) +
  ...
  labs(x = "Prevalence estimate (%)", y = "Number of subgroup estimates", color = "",
...
ggplot(df, aes(x = Age, y = Data_Value, fill = Age)) +
  ...
  labs(x = "Age Group", y = "Crude Prevalence (%)",
