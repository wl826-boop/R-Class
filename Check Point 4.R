...
data_description <- data.frame(
  Variable = c("Sex",
               ...
               "Response",
               "Sample Size"),
  Type_in_Dataset = c("Categorical",
                      "Continuous",
                      "Categorical",
                      "Categorical",
                      "Discrete"),
  Class_in_R = c(class(nhanes_eye$Sex),
...
```{r}
# Build summary dataframe from simulation results
...
x_breaks <- levels(sim_summary$n_per_group)[seq(1, length(levels(sim_summary$n_per_group)), by = 2)]

# Final visualization
ggplot(sim_summary,
       aes(x = as.numeric(as.character(n_per_group)),  # ← numeric, not factor
           y = estimated_difference,
           color = noise_lab)) +
  geom_hline(aes(yintercept = effect_size), linetype = "dashed",
             linewidth = 0.8, color = "black") +
  geom_point(
    alpha = 0.5,
    size = 2.5,
    position = position_jitter(width = 8, height = 0)  # ← jitter in actual units
  ) +
  facet_wrap(~ effect_size, ncol = 5) +
  scale_x_continuous(
    breaks = c(50, 200, 350, 500),  # ← only show a few clean labels
    labels = c("50", "200", "350", "500")
  ) +
  ...
  labs(
    ...
    title = "Estimated Difference Across Effect Sizes, Sample Size, and Noise"
  ) +
